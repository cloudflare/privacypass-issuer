...
export class WshimLogger {
	...
	log(...msg: unknown[]): void {
		...
		if (msg.length === 1 && typeof msg[0] === 'object' && msg[0] !== null) {
			...
			const message =
				typeof structured.message === 'string' ? structured.message : JSON.stringify(structured);
	...
	error(...msg: unknown[]): void {
		...
		const message =
			(structured?.message as string | undefined) ?? maybeError?.message ?? fallbackMessage;
